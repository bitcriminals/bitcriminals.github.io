---
title: PrivateEncryptionMistake
layout: post
author: Dr.DONN4
date: 2021-05-19 17:00:00 +530
type: Crypto
difficulty: Medium/Hard
prompt: 
---

Well,with some research on the net I found a blog https://blog.cryptohack.org/twitter-secrets

Reading this blog I understood the private key is encoded in PEM.So, first of all I used some OCR technques to get the key but somehow that was totally correct as some bytes were wrong.
I had to do a hectic amount of work with eyes and find the corrections :( 

```
-----BEGIN RSA PRIVATE KEY-----

[                           Snipped!                           ]
[                           Snipped!                           ]
[                           Snipped!                           ]




-----END RSA PRIVATE KEY-----
```

Well I converted this in PEM encoding into Hex with cyberchef and started analysing the key.First I looked for the ASN.1 header and it was 02820101 . Gradually,I analysed the first part of the key to find the N_upper bits
and then I anlalysed the lower part of the key to find the rest of needed unknowns-p_lower-bits,q,dp,dq_upper_bits.Basically,the ASN.1 header was used to seperate the parameters from one another.

Finally, we found :

```
N_upper_bits = 
p_lower_bits = 0x644b7a5b0c84a8784fc8fd
q = 
dp = 
dq_upper_bits = 0x7e57e62940d30b891f917c285f31f11e48226a514f6eccefcce9b2077f6adc4da9539a454c286f
```

I assumed e=65537 and with the help of the blog produced a simplified script in python :)

```python3

#!/bin/python3

from Crypto.Util.number import bytes_to_long, isPrime
from sympy import *
from Crypto.PublicKey import RSA

#parameters within the key

N_upper_bits = 
p_lower_bits = 0x644b7a5b0c84a8784fc8fd
q = 
dp = 
dq_upper_bits = 0x7e57e62940d30b891f917c285f31f11e48226a514f6eccefcce9b2077f6adc4da9539a454c286f
e=65537

#finding p

for kp in range(3, e):
    p_mul = dp * e - 1
    if p_mul % kp == 0:
        p = (p_mul // kp) + 1
        if isPrime(p):
            print(f"Possible p: {p}")

#finding N,phi,d 

q=p*q

assert hex(N).startswith(hex(N_upper_bits))

phi = (p-1)*(q-1)
d = pow(e,-1,phi)

#constructing the RSA key
key = RSA.construct((N,e,d,p,q))
pem = key.export_key('PEM')
print(pem.decode())

```

Running the script we get a our private key 

```
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY----- 

```

Now, do ssh login with this key to get the flag

FLAG=dctf{Y0u_Are_N0w_Br34thing_M4nua11y}